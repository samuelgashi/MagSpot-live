...
class STREAM_BY_LIBRARY:
    ...
    def get_all_saved_content_from_library(self, content_type):
        try:
            ...
            # STEP 2: --> Clicking Library Tab...
            update_task(self.user_id, progress=25, task_id=self.task_id, status="RUNNING", log="--> Clicking Library Tab...") 
            try: 
                coordx, coordy, elem_obj = self.YT_MUSIC_TOOL_.perform_action("click_nav_library_button")
                update_task(self.user_id, self.task_id, progress=25, status="RUNNING", log=f"--> Clicking Library Tab [Coords: {coordx}, {coordy}]...")
            except: raise

            # Clicking Library Filter Box
            update_task(self.user_id, self.task_id, progress=27, status="RUNNING", log=f"--> Clicking Playlist Filter From Search Filter...")
            try: 
                if content_type.lower().strip() == "artists":
                    WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, LIBRARY_FILTER_PLAYLISTS_XPATH))).click()
                elif content_type.lower().strip() == "albums":
                    WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, LIBRARY_FILTER_PLAYLISTS_XPATH))).click()
                elif content_type.lower().strip() == "songs":
                    WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, LIBRARY_FILTER_PLAYLISTS_XPATH))).click()
                elif content_type.lower().strip() == "playlists":
                    WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, LIBRARY_FILTER_PLAYLISTS_XPATH))).click()
                elif content_type.lower().strip() == "profiles":
                    WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, LIBRARY_FILTER_PLAYLISTS_XPATH))).click()
                else: raise Exception("No Content Type Specified. [Albums, Artist, Songs, Playlists, Profiles]")
                time.sleep(random.uniform(3.5, 6.5))
            except:  raise Exception(f"--> ERROR: FAILED Clicking {content_type.upper()} Filter From Search Filter...")

            unique_library_saved_content_list = self.YT_MUSIC_TOOL_.get_all_library_saved_content()
            self.YT_MUSIC_TOOL_.switch_to_home_page(Config.YT_MUSIC_PACKAGE_NAME, Config.YT_MUSIC_ACTIVITY_NAME)
            time.sleep(2)
            return unique_library_saved_content_list

        except Exception as e: raise Exception(f"Failed to Get Saved Content From Library [{content_type}]. {e}")

    

    def start_stream_by_library_stream(self, playlist_name, play_hours=10): 
        try:
            ...
            # STEP 3: Clicking Playlist Filter From Search Filter...
            update_task(self.user_id, self.task_id, progress=27, status="RUNNING", log=f"--> Clicking Playlist Filter From Search Filter...")
            try: WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, LIBRARY_FILTER_PLAYLISTS_XPATH))).click()
            except: update_task(self.user_id, self.task_id, progress=27, status="RUNNING", log=f"--> WARNING: FAILED Clicking Playlist Filter From Search Filter...")
            ...
            # STEP 5: Clicking Shuffle From Action Menu...
            try: 
            ...
            # try:
            #     coordx, coordy, elem_obj = self.YT_MUSIC_TOOL_.perform_action("click_play_button_PLAYLIST_SERACH", playlist_name)
            #     update_task(self.user_id, self.task_id, progress=35, status="RUNNING", log=f"--> Clicked 'Play' Button [Coord {coordx}, {coordy}]...")
            ...
            # STEP 6: Clicking 'Shuffle' + Reat Button...
            try: 
            ...
            # update_task(self.user_id, self.task_id, progress=50, status="RUNNING", log=f"--> Skipping First Song...") 
            # next_btn_x, next_btn_y = get_next_btn_coord(screenWidth, screenHeight, self.isOverrideResolution)
            # tapOnScreenCoord(self.driver, next_btn_x, next_btn_y, android_ip=self.device_id)
            # time.sleep(3)


            # STEP 7: Generating Rates And Playing Musics...
            sets = 1
