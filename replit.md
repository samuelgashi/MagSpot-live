...
- **Auth overhaul (Clerk removed):** Clerk authentication removed entirely. `backend/app/utils/auth.py` now uses custom session JWT (HS256 with FLASK_SECRET_KEY) for browser login and existing API-key auth for headless/APIAAS clients. `ADMIN_PASSWORD` env var (default: `admin`) seeds the single admin user at startup. Login endpoint: `POST /api/auth/login`. Password change: `POST /api/auth/change_password`. The Node.js dev backend also provides compatible `/api/auth/*` endpoints so login works in the Replit dev environment.
- **MagSpot 2.0 Login page:** App now shows a custom login page on first load (no session token) and routes to the Dashboard after successful authentication. Token stored in `localStorage.sessionToken`, sent as `Authorization: Bearer` header on all API calls.
- **MagSpot 2.0 Settings panel:** Added Change Password card (current/new/confirm fields) and Logout button.
- **Resources fix:** `ResourceUsage.tsx` now correctly parses both numeric and string (e.g. `"35%"`) percent values returned by the backend.
